"use client";

import { motion } from "framer-motion";
import { Heart, Calendar, Clock, MapPin, ArrowRight, BookOpen, Users } from "lucide-react";
import Link from "next/link";

const eventos = [
  {
    title: "Reunión de Casados",
    date: "Sábados quincenales",
    time: "6:00 pm",
    location: "Sede Lima Centro",
    description: "Tiempo juntos para hablar de lo que importa en tu matrimonio.",
    recurring: true,
  },
  {
    title: "Retiro de Casados",
    date: "22-24 Mayo 2026",
    time: "Salida 6:00 pm viernes",
    location: "Cieneguilla",
    description: "Fin de semana para reconectar como pareja, sin hijos, sin agenda.",
    featured: true,
  },
  {
    title: "Taller: Comunicación en pareja",
    date: "Próximamente",
    time: "Por confirmar",
    location: "Online + Presencial",
    description: "Herramientas prácticas para hablar y escuchar mejor.",
  },
];

const temas = [
  "Comunicación en pareja",
  "Finanzas del hogar",
  "Crianza con propósito",
  "Intimidad y confianza",
  "Conflictos sanos",
  "Fe en familia",
];

export default function CasadosContent() {
  return (
    <>
      {/* Hero */}
      <section className="relative min-h-[70vh] flex items-end pb-20 pt-32 px-6 md:px-10 overflow-hidden">
        <div className="absolute inset-0 z-0">
          <div
            className="absolute inset-0 bg-cover bg-center"
            style={{ backgroundImage: "url(https://images.unsplash.com/photo-1529539795054-3c162aab037a?w=1920&q=80)" }}
          />
          <div className="absolute inset-0 bg-gradient-to-t from-[var(--bg)] via-[var(--bg)]/60 to-[var(--bg)]/30" />
        </div>

        <div className="relative z-10 max-w-[1600px] mx-auto w-full">
          <motion.span
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8 }}
            className="inline-flex items-center gap-2 font-mono text-[11px] tracking-[0.32em] text-[var(--fg-60)] uppercase mb-6"
          >
            <Heart size={14} strokeWidth={1.5} />
            Ministerio · Matrimonios
          </motion.span>

          <motion.h1
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 1, delay: 0.1 }}
            className="display-heading text-[var(--fg)] text-[clamp(2.5rem,7vw,5.5rem)] mb-6"
          >
            Construir juntos
            <br />
            <em className="italic font-light text-[var(--fg-70)]">lo que dure.</em>
          </motion.h1>

          <motion.p
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            transition={{ duration: 0.8, delay: 0.2 }}
            className="text-[var(--fg-60)] text-lg md:text-xl leading-relaxed max-w-xl"
          >
            Sin frases hechas. Un espacio para parejas que quieren crecer juntas,
            con herramientas reales y acompañamiento de quienes ya recorrieron el camino.
          </motion.p>
        </div>
      </section>

      {/* What we offer */}
      <section className="bg-[var(--bg)] py-24 px-6 md:px-10 border-t border-[var(--line)]">
        <div className="max-w-[1600px] mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
            {/* Left: topics */}
            <div>
              <span className="font-mono text-[11px] tracking-[0.32em] text-[var(--fg-50)] uppercase">
                Temas que abordamos
              </span>
              <h2 className="font-display text-4xl text-[var(--fg)] mt-4 mb-8">
                Lo que importa en tu matrimonio.
              </h2>
              <div className="grid grid-cols-2 gap-3">
                {temas.map((tema) => (
                  <div
                    key={tema}
                    className="px-4 py-3 rounded-lg bg-[var(--surface)] border border-[var(--line)] text-[var(--fg-70)] text-sm"
                  >
                    {tema}
                  </div>
                ))}
              </div>
            </div>

            {/* Right: study CTA */}
            <div className="bg-[var(--accent-warm-10)] border border-[var(--accent-warm-30)] rounded-2xl p-8 md:p-10 flex flex-col justify-between">
              <div>
                <BookOpen size={24} strokeWidth={1.5} className="text-[var(--accent-warm)] mb-4" />
                <h3 className="font-display text-3xl text-[var(--fg)] mb-4">
                  Estudio bíblico para parejas
                </h3>
                <p className="text-[var(--fg-60)] text-sm leading-relaxed mb-6">
                  Agenda un estudio con un líder especializado en matrimonios.
                  Pueden ir como pareja o individualmente. Los temas se adaptan
                  a lo que ustedes necesitan.
                </p>
              </div>
              <Link
                href="/estudios"
                className="group inline-flex items-center gap-3 px-8 py-4 rounded-full bg-[var(--accent-warm)] text-white text-sm font-medium hover:opacity-90 transition-all self-start"
              >
                Agendar estudio
                <ArrowRight size={16} strokeWidth={1.5} className="group-hover:translate-x-1 transition-transform" />
              </Link>
            </div>
          </div>
        </div>
      </section>

      {/* Events */}
      <section className="bg-[var(--bg)] py-24 px-6 md:px-10 border-t border-[var(--line)]">
        <div className="max-w-[1600px] mx-auto">
          <div className="flex items-center gap-4 mb-12">
            <span className="font-mono text-[11px] tracking-[0.32em] text-[var(--fg-50)] uppercase">
              Próximos eventos
            </span>
            <div className="flex-1 h-px bg-[var(--line)]" />
          </div>

          <div className="space-y-4">
            {eventos.map((event, i) => (
              <motion.div
                key={event.title}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                transition={{ duration: 0.5, delay: i * 0.05 }}
                className={`group p-6 md:p-8 rounded-xl border transition-all ${
                  event.featured
                    ? "bg-[var(--accent-warm-10)] border-[var(--accent-warm-30)]"
                    : "bg-[var(--surface)] border-[var(--line)] hover:border-[var(--line-strong)]"
                }`}
              >
                <div className="flex flex-col md:flex-row md:items-center gap-6">
                  <div className="flex-1 min-w-0">
                    <h3 className="font-display text-2xl text-[var(--fg)] mb-2 group-hover:italic transition-all">
                      {event.title}
                    </h3>
                    <p className="text-[var(--fg-50)] text-sm">{event.description}</p>
                  </div>
                  <div className="flex flex-wrap items-center gap-4 text-[var(--fg-50)] text-sm shrink-0">
                    <span className="inline-flex items-center gap-1.5">
                      <Calendar size={14} strokeWidth={1.5} className="text-[var(--fg-30)]" />
                      {event.date}
                    </span>
                    <span className="inline-flex items-center gap-1.5">
                      <Clock size={14} strokeWidth={1.5} className="text-[var(--fg-30)]" />
                      {event.time}
                    </span>
                    <span className="inline-flex items-center gap-1.5">
                      <MapPin size={14} strokeWidth={1.5} className="text-[var(--fg-30)]" />
                      {event.location}
                    </span>
                  </div>
                </div>
              </motion.div>
            ))}
          </div>
        </div>
      </section>

      {/* CTA */}
      <section className="bg-[var(--bg)] py-24 px-6 md:px-10 border-t border-[var(--line)]">
        <div className="max-w-3xl mx-auto text-center">
          <motion.h2
            initial={{ opacity: 0, y: 30 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ duration: 1 }}
            className="display-heading text-[var(--fg)] text-[clamp(2.5rem,6vw,4.5rem)] mb-6"
          >
            Tu matrimonio merece
            <em className="italic font-light text-[var(--fg-70)]"> inversión.</em>
          </motion.h2>

          <p className="text-[var(--fg-60)] text-lg mb-10">
            Conéctate con otras parejas o agenda un estudio personalizado para tu matrimonio.
          </p>

          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <Link
              href="/estudios"
              className="group inline-flex items-center gap-3 px-8 py-4 rounded-full bg-[var(--accent-warm)] text-white text-sm font-medium hover:opacity-90 transition-all"
            >
              Agendar estudio para parejas
              <ArrowRight size={16} strokeWidth={1.5} className="group-hover:translate-x-1 transition-transform" />
            </Link>
            <a
              href="https://wa.me/51999999999?text=Hola! Queremos unirnos al ministerio de Casados"
              target="_blank"
              rel="noopener noreferrer"
              className="group inline-flex items-center gap-3 px-8 py-4 rounded-full border border-[var(--line-strong)] text-[var(--fg)] text-sm font-medium hover:bg-[var(--surface-10)] transition-all"
            >
              Contactar al equipo
              <ArrowRight size={16} strokeWidth={1.5} className="group-hover:translate-x-1 transition-transform" />
            </a>
          </div>
        </div>
      </section>
    </>
  );
}
